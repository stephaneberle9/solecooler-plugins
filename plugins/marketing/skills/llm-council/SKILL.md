...
- `company-context` — optional. Load when the question is about a Solecooler product, offer, audience, or competitive position.
...
After delivering the HTML report and markdown transcript, ask before taking any of the following steps — never act without explicit confirmation:

- **`~~cloud-storage`** — Save the HTML report to Google Drive. Ask: which folder?
- **`~~chat`** — Share a summary or the report link to a Slack channel. Ask: which channel?
- **`~~email`** — Email the report to decision-makers. Ask: which address(es)?

Prompt: "Should I save the report to Drive, share it to Slack, send it by email, or keep it here for now?"
